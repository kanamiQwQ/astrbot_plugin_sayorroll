...
class SayoRoll(Star):
    ...
    async def on_roll(self, event: AstrMessageEvent, message: str = ""):
        ...
        if not args_text:
        ...
        elif result := re.search(r'([\u4e00-\u9fff])[不没]\1(.*?)', args_text):
            options = [result.group()[:1], result.group()[1:]]
            
            first_params = args_text[:result.span()[0]].replace('我', '你')
            second_params = args_text[result.span()[1]:].replace('我', '你')
            ...
            if similarity > 0.8:
                ...
                msg = f'当然是{random.choice(options)}咯'.replace('我', '你')
                
        else:
            args_text = self.normalize_str(args_text)
            
            options = args_text.split()
